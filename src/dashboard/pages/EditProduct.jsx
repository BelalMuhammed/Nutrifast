...
// ✅ Simple SVG Icons (same as AddProducts)
...
function CheckboxDropdown({ label, options, registerName, register }) {
    ...
    return (
        <div className='relative w-full'>
            ...
            <div
                className='w-full border-2 border-gray-300 rounded-xl p-2 bg-gray-50 cursor-pointer 
                   hover:border-green-600 hover:bg-white transition-all duration-300 flex 
            ...
            {open && (
                <div className='absolute z-20 mt-2 w-full bg-white border-2 border-gray-300 
                        rounded-xl shadow max-h-48 overflow-y-auto'>
                        ...
                        {options.map((opt) => (
                           ...
                           rounded-lg cursor-pointer transition-colors duration-200'>
                                <input
                                    ...
                                    className='w-5 h-5 text-app-primary bg-gray-100 border-2 
                             border-gray-300 rounded focus:ring-green-500 focus:ring-2'
                                />
    ...
    return (
        <Controller
            ...
            render={({ field }) => (
                <div>
                    <label className='block text-sm font-bold text-gray-800 mb-3'>{label}</label>
                    <div className='flex flex-wrap gap-3 border-2 border-gray-300 rounded-xl p-2 
                          bg-gray-50 min-h-[60px] focus-within:border-green-600 
                          focus-within:bg-white transition-all duration-300'>
                        {field.value.map((tag, idx) => (
                            <span
                                key={idx}
                                className='bg-app-primary text-white px-4 py-2 rounded-full text-sm 
                           font-medium flex items-center gap-2 shadow-md'>
                                {tag}
                                <button
                                    type='button'
                        ...
                        <input
                            ...
                            className='flex-1 outline-none bg-transparent text-sm placeholder-gray-600 
...
export default function EditProduct() {
    ...
    useEffect(() => {
        async function fetchProduct() {
            try {
                const res = await fetch(`https://nutrifast-data.up.railway.app/products/${id}`);
                    ...
                    <div className='flex items-center gap-4 mb-2'>
                        ...
                        <div>
                            ...
                            <p className='text-gray-600 text-sm sm:text-base font-medium'>
                                Update product details for your Nutrifast store
                            </p>
                        </div>
                    </div>
                </div>

                {/* ✅ The form (copied from AddProducts but prefilled via reset) */}
                <form
