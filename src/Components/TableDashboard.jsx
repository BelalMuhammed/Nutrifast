...
export default function TableDashboard({
  ...
  return (
    <div className='space-y-6 bg-gradient-to-br from-gray-50 via-white to-gray-100 min-h-screen pt-4 sm:pt-6 lg:pt-8 px-4 sm:px-6 lg:px-8'>
      ...
      {/* Table (desktop only) */}
      <div className='bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 overflow-hidden hidden md:block'>
                ...
                {table.getRowModel().rows.length ? (
                  table.getRowModel().rows.map((row, index) => (
                    <TableRow
                      ...
                      className={`hover:bg-gradient-to-r hover:from-app-primary/5 hover:to-app-secondary/5 transition-all duration-300 ${
                        row.getIsSelected()
                ...
                <div className='flex items-center gap-2 sm:gap-3'>
                  <div
                    className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${
                      row.getIsSelected() ? "bg-app-primary" : "bg-gray-300"
                    }`}></div>
